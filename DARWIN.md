...
During every switch, nix-darwin runs `brew update` so Homebrew can report available updates, but does not upgrade installed formulae, casks, or Mac App Store apps automatically. Run `brew outdated` to review them and `brew upgrade` when you want to install updates. Self-updating or unversioned casks are intentionally not upgraded (`greedyCasks = false`), since those applications manage their own updates. To hold a specific installed cask version, use `brew pin --cask <name>`; this is machine-local state and does not prevent the application from updating itself. Notes:

- The first switch after adding new packages can take a while while Homebrew refreshes its package index and reconciles the Brewfile.
