...
local function blink_accept_with_autopairs_and_luasnip()
	...
	if ok_ap and cmp_ap and type(cmp_ap.on_confirm_done) == "function" then
		...
		if ok_h and type(handler) == "function" then
			autopairs_handler = handler
		end
	end
	...
	vim.schedule(function()
		...
		if selected_entry then
			if type(selected_entry.get_completion_item) == "function" then
				pcall(function()
					completion_item = selected_entry:get_completion_item()
				end)
		...
		if ok_ls and completion_item then
			...
			local is_snippet = completion_item.insertTextFormat == 2 or (body and tostring(body):match("%%${")) -- Crude snippet marker detection
