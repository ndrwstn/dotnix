{ config, options, pkgs, lib, autopkgs, ... }:

{
  ...
  config =
    let
      ...
      isAppleSiliconDarwin = config._astn.machine.system == "aarch64-darwin";
    in
    lib.mkIf cfg.enable (lib.mkMerge [
      ...
      (lib.mkIf cfg.mlxLm.enable {
        environment.systemPackages = [ autopkgs."mlx-lm" ];
      })
      (lib.mkIf (hasHomebrew && cfg.lmStudio.enable) {
